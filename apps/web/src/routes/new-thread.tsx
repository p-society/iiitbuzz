...
export default function NewThreadPage() {
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		try {
			...
			if (isAnonymous) {
				toast.success("Thread submitted for admin approval!");
			} else {
		...
		<div className="min-h-screen bg-background pb-12">
			...
			<header className="border-b-4 border-black bg-card p-6">
				<div className="mx-auto max-w-4xl">
					<Link
						to="/home"
						className="mb-4 inline-flex items-center gap-2 font-bold text-primary text-sm hover:underline"
					>
			...
			<main className="mx-auto max-w-4xl px-4 py-8">
				<form onSubmit={handleSubmit} className="space-y-8">
					...
					<section>
						<label className="mb-2 block font-bold text-xl">
							2. Thread Title
						</label>
						<input
					...
					<section>
						...
						<p className="text-[10px] text-muted-foreground mt-1">
							Anonymous threads require admin approval before being visible to
							others.
						</p>
					</section>
					...
					<div className="flex flex-col sm:flex-row gap-4 pt-4">
						...
						<Link
							to="/home"
							className="neo-brutal-button bg-white px-10 py-4 font-bold text-lg text-center"
						>
							Cancel
						</Link>
					</div>
